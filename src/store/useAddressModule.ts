...
const initialState: Address | AddressCreateInput | AddressUpdateInput = {
  ...
  town: "",
  zipCode: "",
  addressOfActorObject: { id: "" },
  listIdCountryObject: { id: "" },
...
export const useAddressStore = defineStore("address-store", {
  ...
  actions: {
    ...
    async createAddress(payload?: { data: AddressCreateInput }) {
      ...
      try {
        const address = payload?.data ?? (this.address as AddressCreateInput)
        const newAddress = {
          ...address, listIdCountryObject: address.listIdCountryObject?.id.length ? address.listIdCountryObject : undefined
          , addressType: address.addressType?.id.length ? address.addressType : undefined
        }

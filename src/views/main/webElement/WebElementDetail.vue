  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="webelement">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(webelement)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="webelementDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { getWebElementById } = useWebElementStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getWebElementById(id);
  setCurrentPageBreadcrumbs("webelements", ["detail"]);
});
</script>
<style>
.bottom {

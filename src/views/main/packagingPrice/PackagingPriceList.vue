  ...
  <div class="mx-auto py-5 ms-4 me-4 h-100">
    ...
    >
      <el-button
        color="#606266"
        class="button__excel"
        :disabled="packagingpriceList.length === 0"
        @click="dataToExcel"
        >Export to EXCEL</el-button
      >
      <el-table
        :data="packagingpriceList"
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`packagingpriceList_${
                  packagingpriceList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`packagingpriceList${
                fieldsName[scope.$index]
              }EditLink_${packagingpriceList[scope.$index].id}`"
            ...
            <el-button
              :data-test="`packagingpriceList${
                fieldsName[scope.$index]
              }DeleteBtn_${packagingpriceList[scope.$index].id}`"
              @click="
                () =>
                  handleOpenConfirmModal(packagingpriceList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === packagingpriceList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(packagingpriceList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper mt-auto">
        <Pagination
          :itemsPerPage="packagingpricePagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(
              packagingpricePagination.skip / packagingpricePagination.take
            ) + 1
          "
          :current-page-change="currentPageChange"
          :total="packagingpricePagination.total"
...
const entityPluralName = "packagingprices";
const fieldsName = ref([
  "price",
  "validFrom",
  "validUntil",
  "listIdCurrencyObject",
  "packaging",
]);
...
const {
  ...
  packagingpriceExcelFile,
} = storeToRefs(usePackagingPriceStore());
const {
  fetchPackagingPrices,
  deletePackagingPrice,
  fetchDataExcelPackagingPrices,
} = usePackagingPriceStore();

async function dataToExcel() {
  await fetchDataExcelPackagingPrices();
  const workbook = XLSX.read(packagingpriceExcelFile.value.toString());
  XLSX.writeFile(workbook, "packagingpriceData.xlsx");
}
...
const currentPageChange = async (val: number) => {
  await fetchPackagingPrices({
    skip: (val - 1) * packagingpricePagination.value.take,
    take: packagingpricePagination.value.take,
  });
};
onMounted(async () => {

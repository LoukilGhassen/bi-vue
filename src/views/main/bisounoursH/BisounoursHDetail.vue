  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="bisounoursh">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(bisounoursh)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="bisounourshDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { bisounoursh } = storeToRefs(useBisounoursHStore());
const { getBisounoursHById } = useBisounoursHStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getBisounoursHById(id);
  setCurrentPageBreadcrumbs("bisounourshes", ["detail"]);
});
</script>
<style>
.bottom {

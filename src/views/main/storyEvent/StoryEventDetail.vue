  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="storyevent">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(storyevent)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="storyeventDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { storyevent } = storeToRefs(useStoryEventStore());
const { getStoryEventById } = useStoryEventStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getStoryEventById(id);
  setCurrentPageBreadcrumbs("storyevents", ["detail"]);
});
</script>
<style>
.bottom {

  ...
  <div class="mx-auto py-5 ms-4 me-4 h-100">
    ...
    >
      <el-button
        color="#606266"
        class="button__excel"
        :disabled="storyeventList.length === 0"
        @click="dataToExcel"
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`storyeventList_${storyeventList[scope.$index].id}`"
                :to="`/${currentUser.role}/${entityPluralName}/${
                  storyeventList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`storyeventList${fieldsName[scope.$index]}EditLink_${
                storyeventList[scope.$index].id
              }`"
            ...
            <el-button
              :data-test="`storyeventList${fieldsName[scope.$index]}DeleteBtn_${
                storyeventList[scope.$index].id
              }`"
              @click="
                () => handleOpenConfirmModal(storyeventList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === storyeventList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(storyeventList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper mt-auto">
        <Pagination
          :itemsPerPage="storyeventPagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(storyeventPagination.skip / storyeventPagination.take) +
            1
          "
          :current-page-change="currentPageChange"
          :total="storyeventPagination.total"
...
const entityPluralName = "storyevents";
...
const { storyeventList, storyeventPagination, storyeventExcelFile } =
  storeToRefs(useStoryEventStore());
const { fetchStoryEvents, deleteStoryEvent, fetchDataExcelStoryEvents } =
  useStoryEventStore();

async function dataToExcel() {
  await fetchDataExcelStoryEvents();
  const workbook = XLSX.read(storyeventExcelFile.value.toString());
  XLSX.writeFile(workbook, "storyeventData.xlsx");
}
...
const currentPageChange = async (val: number) => {
  await fetchStoryEvents({
    skip: (val - 1) * storyeventPagination.value.take,
    take: storyeventPagination.value.take,
  });
};
onMounted(async () => {

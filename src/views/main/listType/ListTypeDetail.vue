  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="listtype">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(listtype)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="listtypeDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { listtype } = storeToRefs(useListTypeStore());
const { getListTypeById } = useListTypeStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getListTypeById(id);
  setCurrentPageBreadcrumbs("listtypes", ["detail"]);
});
</script>
<style>
.bottom {

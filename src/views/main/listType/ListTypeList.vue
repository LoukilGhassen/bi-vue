  ...
  <div class="mx-auto py-5 ms-4 me-4 h-100">
    ...
    >
      <el-button
        color="#606266"
        class="button__excel"
        :disabled="listtypeList.length === 0"
        @click="dataToExcel"
      ...
      >
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`listtypeList${fieldsName[scope.$index]}EditLink_${
                listtypeList[scope.$index].listTypeName
              }`"
            ...
            <el-button
              :data-test="`listtypeList${fieldsName[scope.$index]}DeleteBtn_${
                listtypeList[scope.$index].listTypeName
              }`"
              @click="
                () =>
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal &&
                modalId === listtypeList[scope.$index].listTypeName
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () =>
                  handleRemoveEntity(listtypeList[scope.$index]?.listTypeName)
              "
            >
      ...
      <div class="footer__wrapper mt-auto">
        <Pagination
          :itemsPerPage="listtypePagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(listtypePagination.skip / listtypePagination.take) + 1
          "
          :current-page-change="currentPageChange"
          :total="listtypePagination.total"
...
const entityPluralName = "listtypes";
const fieldsName = ref(["listTypeName"]);
const isOpenModal = ref(false);
const modalId = ref<string | null>(null);
const { isLoading } = storeToRefs(useBodyStore());
const { listtypeList, listtypePagination, listtypeExcelFile } = storeToRefs(
  useListTypeStore()
);
const { fetchListTypes, deleteListType, fetchDataExcelListTypes } =
  useListTypeStore();

async function dataToExcel() {
  await fetchDataExcelListTypes();
  const workbook = XLSX.read(listtypeExcelFile.value.toString());
  XLSX.writeFile(workbook, "listtypeData.xlsx");
}
const deleteLoading = ref<boolean>(false);
const handleRemoveEntity = async (id: string) => {
  deleteLoading.value = true;
  await deleteListType(id);
...
const currentPageChange = async (val: number) => {
  await fetchListTypes({
    skip: (val - 1) * listtypePagination.value.take,
    take: listtypePagination.value.take,
  });
};
onMounted(async () => {

  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="bisounousjrole">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(bisounousjrole)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="bisounousjroleDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { bisounousjrole } = storeToRefs(useBisounousJRoleStore());
const { getBisounousJRoleById } = useBisounousJRoleStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getBisounousJRoleById(id);
  setCurrentPageBreadcrumbs("bisounousjroles", ["detail"]);
});
</script>
<style>
.bottom {

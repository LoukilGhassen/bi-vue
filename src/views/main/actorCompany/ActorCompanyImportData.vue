    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`actorcompanyImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="actorcompanyImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="actorcompanyImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useActorCompanyStore());
const { createManyActorCompany } = useActorCompanyStore();
...
const initialState = {
  companyObject: "",
  shortName: "",
  listIdActorCompanyForm: "",
  listIdActorCompanyType: "",
  actorId: "",
};
...
const actorcompanyInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      actorcompanyInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      actorcompanyInsertData.value = actorcompanyInsertData.value.map(
        (item: any) => {
          let res: any = {};
          for (let key in rest) {
      ...
      await createManyActorCompany(actorcompanyInsertData.value);

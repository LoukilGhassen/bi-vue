  ...
  <div class="mx-auto py-5 ms-4 me-4 h-100">
    ...
    >
      <el-button
        color="#606266"
        class="button__excel"
        :disabled="actorcompanyList.length === 0"
        @click="dataToExcel"
        >Export to EXCEL</el-button
      >
      <el-table
        :data="actorcompanyList"
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`actorcompanyList_${
                  actorcompanyList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`actorcompanyList${
                fieldsName[scope.$index]
              }EditLink_${actorcompanyList[scope.$index].id}`"
            ...
            <el-button
              :data-test="`actorcompanyList${
                fieldsName[scope.$index]
              }DeleteBtn_${actorcompanyList[scope.$index].id}`"
              @click="
                () => handleOpenConfirmModal(actorcompanyList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === actorcompanyList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(actorcompanyList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper mt-auto">
        <Pagination
          :itemsPerPage="actorcompanyPagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(
              actorcompanyPagination.skip / actorcompanyPagination.take
            ) + 1
          "
          :current-page-change="currentPageChange"
          :total="actorcompanyPagination.total"
...
const entityPluralName = "actorcompanies";
...
const { actorcompanyList, actorcompanyPagination, actorcompanyExcelFile } =
  storeToRefs(useActorCompanyStore());
const {
  fetchActorCompanies,
  deleteActorCompany,
  fetchDataExcelActorCompanies,
} = useActorCompanyStore();

async function dataToExcel() {
  await fetchDataExcelActorCompanies();
  const workbook = XLSX.read(actorcompanyExcelFile.value.toString());
  XLSX.writeFile(workbook, "actorcompanyData.xlsx");
}
const deleteLoading = ref<boolean>(false);
const handleRemoveEntity = async (id: string) => {
  deleteLoading.value = true;
  await deleteActorCompany(id);
...
const currentPageChange = async (val: number) => {
  await fetchActorCompanies({
    skip: (val - 1) * actorcompanyPagination.value.take,
    take: actorcompanyPagination.value.take,
  });
};
onMounted(async () => {

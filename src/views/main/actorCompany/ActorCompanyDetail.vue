  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actorcompany">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actorcompany)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actorcompanyDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { actorcompany } = storeToRefs(useActorCompanyStore());
const { getActorCompanyById } = useActorCompanyStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getActorCompanyById(id);
  setCurrentPageBreadcrumbs("actorcompanies", ["detail"]);
});
</script>
<style>
.bottom {

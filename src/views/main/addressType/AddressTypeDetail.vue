  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="addresstype">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(addresstype)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="addresstypeDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { addresstype } = storeToRefs(useAddressTypeStore());
const { getAddressTypeById } = useAddressTypeStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getAddressTypeById(id);
  setCurrentPageBreadcrumbs("addresstypes", ["detail"]);
});
</script>
<style>
.bottom {

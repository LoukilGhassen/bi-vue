    ...
    >
      ...
      >
        ...
        <el-table-column
          ...
          :key="fieldName"
          :prop="fieldName"
          :label="fieldName.charAt(0).toUpperCase() + fieldName.slice(1).replaceAll('Object','').replaceAll('OfActor','').replaceAll('istId','ist').replaceAll('ddress','ddres')"
        />
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`addressList${fieldsName[scope.$index]}EditLink_${
                addressList[scope.$index].id
              }`"
            ...
            <el-button
              :data-test="`addressList${fieldsName[scope.$index]}DeleteBtn_${
                addressList[scope.$index].id
              }`"
              @click="
...
const { fetchAddresses, deleteAddress, fetchDataExcelAddresses } =
  useAddressStore();

  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actorperson">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actorperson)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actorpersonDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { getActorPersonById } = useActorPersonStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getActorPersonById(id);
  setCurrentPageBreadcrumbs("actorpeople", ["detail"]);
});
</script>
<style>
.bottom {

    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`actorpersonImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="actorpersonImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="actorpersonImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useActorPersonStore());
const { createManyActorPerson } = useActorPersonStore();
...
const initialState = {
  ...
  externalUserId: "",
  actorId: "",
  listIdActorPersonTitle: "",
  listIdActorPersonType: "",
  listIdGenre: "",
};
...
const actorpersonInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      actorpersonInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      actorpersonInsertData.value = actorpersonInsertData.value.map(
        (item: any) => {
          let res: any = {};
          for (let key in rest) {
      ...
      await createManyActorPerson(actorpersonInsertData.value);
      ...
      if (!!error.value) {
        ...
        router.push({ name: `${currentUser.value.role}-list-actorperson` });
      }
    } else {

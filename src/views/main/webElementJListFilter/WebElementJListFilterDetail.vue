  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="webelementjlistfilter">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(webelementjlistfilter)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="webelementjlistfilterDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { webelementjlistfilter } = storeToRefs(useWebElementJListFilterStore());
const { getWebElementJListFilterById } = useWebElementJListFilterStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getWebElementJListFilterById(id);
  setCurrentPageBreadcrumbs("webelementjlistfilters", ["detail"]);
});
</script>
<style>
.bottom {

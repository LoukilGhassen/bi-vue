  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="ids">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(ids)"
            :key="index"
          >
...
const { ids } = storeToRefs(useIdsStore());
const { getIdsById } = useIdsStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getIdsById(id);
  setCurrentPageBreadcrumbs("ids", ["detail"]);
});
</script>
<style>
.bottom {

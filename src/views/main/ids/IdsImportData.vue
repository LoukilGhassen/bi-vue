    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`idsImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="idsImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="idsImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useIdsStore());
const { createManyIds } = useIdsStore();
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      idsInsertData.value = idsInsertData.value.map((item: any) => {
        let res: any = {};
        for (let key in rest) {
      ...
      await createManyIds(idsInsertData.value);
      ...
      if (!!error.value) {
        ...
        router.push({ name: `${currentUser.value.role}-list-ids` });
      }
    } else {

  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actoractororientation">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actoractororientation)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actoractororientationDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { actoractororientation } = storeToRefs(useActorActorOrientationStore());
const { getActorActorOrientationById } = useActorActorOrientationStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getActorActorOrientationById(id);
  setCurrentPageBreadcrumbs("actoractororientations", ["detail"]);
});
</script>
<style>
.bottom {

  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actorrelationship">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actorrelationship)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actorrelationshipDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { actorrelationship } = storeToRefs(useActorRelationshipStore());
const { getActorRelationshipById } = useActorRelationshipStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getActorRelationshipById(id);
  setCurrentPageBreadcrumbs("actorrelationships", ["detail"]);
});
</script>
<style>
.bottom {

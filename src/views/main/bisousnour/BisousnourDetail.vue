  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="bisousnour">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(bisousnour)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="bisousnourDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { bisousnour } = storeToRefs(useBisousnourStore());
const { getBisousnourById } = useBisousnourStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getBisousnourById(id);
  setCurrentPageBreadcrumbs("bisousnours", ["detail"]);
});
</script>
<style>
.bottom {

  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="packaging">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(packaging)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="packagingDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { packaging } = storeToRefs(usePackagingStore());
const { getPackagingById } = usePackagingStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getPackagingById(id);
  setCurrentPageBreadcrumbs("packagings", ["detail"]);
});
</script>
<style>
.bottom {

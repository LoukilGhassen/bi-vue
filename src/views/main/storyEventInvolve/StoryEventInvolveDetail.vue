  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="storyeventinvolve">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(storyeventinvolve)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="storyeventinvolveDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { storyeventinvolve } = storeToRefs(useStoryEventInvolveStore());
const { getStoryEventInvolveById } = useStoryEventInvolveStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getStoryEventInvolveById(id);
  setCurrentPageBreadcrumbs("storyeventinvolves", ["detail"]);
});
</script>
<style>
.bottom {

  ...
  <div class="mx-auto py-5 ms-4 me-4 h-100">
    ...
    >
      <el-button
        color="#606266"
        class="button__excel"
        :disabled="actortelecomList.length === 0"
        @click="dataToExcel"
        >Export to EXCEL</el-button
      >
      <el-table
        :data="actortelecomList"
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`actortelecomList_${
                  actortelecomList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`actortelecomList${
                fieldsName[scope.$index]
              }EditLink_${actortelecomList[scope.$index].id}`"
            ...
            <el-button
              :data-test="`actortelecomList${
                fieldsName[scope.$index]
              }DeleteBtn_${actortelecomList[scope.$index].id}`"
              @click="
                () => handleOpenConfirmModal(actortelecomList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === actortelecomList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(actortelecomList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper mt-auto">
        <Pagination
          :itemsPerPage="actortelecomPagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(
              actortelecomPagination.skip / actortelecomPagination.take
            ) + 1
          "
          :current-page-change="currentPageChange"
          :total="actortelecomPagination.total"
...
const entityPluralName = "actortelecoms";
...
const { actortelecomList, actortelecomPagination, actortelecomExcelFile } =
  storeToRefs(useActorTelecomStore());
const { fetchActorTelecoms, deleteActorTelecom, fetchDataExcelActorTelecoms } =
  useActorTelecomStore();

async function dataToExcel() {
  await fetchDataExcelActorTelecoms();
  const workbook = XLSX.read(actortelecomExcelFile.value.toString());
  XLSX.writeFile(workbook, "actortelecomData.xlsx");
}
const deleteLoading = ref<boolean>(false);
const handleRemoveEntity = async (id: string) => {
  deleteLoading.value = true;
  await deleteActorTelecom(id);
...
const currentPageChange = async (val: number) => {
  await fetchActorTelecoms({
    skip: (val - 1) * actortelecomPagination.value.take,
    take: actortelecomPagination.value.take,
  });
};
onMounted(async () => {

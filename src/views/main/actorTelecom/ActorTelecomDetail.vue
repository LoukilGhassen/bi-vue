  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actortelecom">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actortelecom)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actortelecomDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { actortelecom } = storeToRefs(useActorTelecomStore());
const { getActorTelecomById } = useActorTelecomStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getActorTelecomById(id);
  setCurrentPageBreadcrumbs("actortelecoms", ["detail"]);
});
</script>
<style>
.bottom {

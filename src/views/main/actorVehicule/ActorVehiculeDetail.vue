  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actorvehicule">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actorvehicule)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actorvehiculeDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { actorvehicule } = storeToRefs(useActorVehiculeStore());
const { getActorVehiculeById } = useActorVehiculeStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getActorVehiculeById(id);
  setCurrentPageBreadcrumbs("actorvehicules", ["detail"]);
});
</script>
<style>
.bottom {

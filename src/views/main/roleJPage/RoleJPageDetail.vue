  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="rolejpage">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(rolejpage)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="rolejpageDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { rolejpage } = storeToRefs(useRoleJPageStore());
const { getRoleJPageById } = useRoleJPageStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getRoleJPageById(id);
  setCurrentPageBreadcrumbs("rolejpages", ["detail"]);
});
</script>
<style>
.bottom {

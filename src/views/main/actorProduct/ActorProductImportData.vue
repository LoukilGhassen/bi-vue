    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`actorproductImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="actorproductImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="actorproductImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useActorProductStore());
const { createManyActorProduct } = useActorProductStore();
...
const actorproductInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      actorproductInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      actorproductInsertData.value = actorproductInsertData.value.map(
        (item: any) => {
          let res: any = {};
          for (let key in rest) {
      ...
      await createManyActorProduct(actorproductInsertData.value);
      ...
      if (!!error.value) {
        ...
        router.push({ name: `${currentUser.value.role}-list-actorproduct` });
      }
    } else {

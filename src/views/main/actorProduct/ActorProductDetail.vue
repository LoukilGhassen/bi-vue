  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actorproduct">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actorproduct)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actorproductDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { actorproduct } = storeToRefs(useActorProductStore());
const { getActorProductById } = useActorProductStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getActorProductById(id);
  setCurrentPageBreadcrumbs("actorproducts", ["detail"]);
});
</script>
<style>
.bottom {

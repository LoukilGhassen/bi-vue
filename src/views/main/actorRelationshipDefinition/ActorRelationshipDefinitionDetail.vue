  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="actorrelationshipdefinition">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(actorrelationshipdefinition)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="actorrelationshipdefinitionDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { actorrelationshipdefinition } = storeToRefs(
...
onMounted(async () => {
  ...
  setCurrentPageBreadcrumbs("actorrelationshipdefinitions", ["detail"]);
});
</script>
<style>
.bottom {

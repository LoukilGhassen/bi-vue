<template>
  ...
  >
    ...
    >
      <ConfirmSignUpModal :email="form.email" />
    </el-dialog>
...
import ConfirmSignUpModal from "@/components/modals/ConfirmSignUpModal.vue";
  ...
  setup() {
    ...
    const signUp = async () => {
      try {
        ...
        if (!res?.error) confirmEmail.value = true;
      } catch (error) {

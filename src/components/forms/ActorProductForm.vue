<template>
  <div >
      <div class="d-flex justify-content-between align-items-center p-8 pt-10 noir">
        <div class="fs-1 fw-boldest px-4 noir">
          <span v-if="isEdit">{{props.product.name}}</span>
      ...
      <el-form :model="form" ref="ruleFormRef" class="px-9 mx-9" :rules="rules">
        <div class="form fit-content d-flex flex-column">
          ...
          <el-form-item class="w-100" prop="listIdProductCategoryObject.id">
            ...
            v-model="form.listIdProductCategoryObject.id"
          >
            <el-option
          ...
          >
              ...
              <el-select
                  id="market-select"
                  :disabled="!form.listIdProductCategoryObject.id"
                  size="large"
                  class="w-100"
...
const { createActorProduct , editActorProduct} = useActorProductStore();
...
const form = ref({
  name: "",
  code: "",
  listIdProductCategoryObject: {id:""},
  markets: [],
});
const markets = ref()
const rules = {
...
const handleSubmitForm = async () => {
  if(props.isEdit){
    const data = {
      ...
      listIdProductCategoryObject: form.value.listIdProductCategoryObject,
      markets: form.value.markets,
    }
    await editActorProduct({id:form.value.id,data});
  }
  else{
    await createActorProduct({data:form.value})
...
onMounted(async() => {
  ...
  if(props.isEdit){
    ...
    await handleCategorieChange(form.value.listIdProductCategoryObject?.id,true)
  }else {
    const result = await service.api.actorProductControllerGenerateCode()

  ...
  <div class="w-100">
    <div class="d-flex justify-content-between p-9 m-9 mt-5 noir">
      <div class="fs-2 fw-boldest noir">
        <span v-if="isEdit">Modifier</span><span v-else>Ajouter</span> une
    ...
    <el-form :model="form" ref="ruleFormRef" class="px-9 mx-9" :rules="rules">
      <div class="form fit-content d-flex flex-column">
        <div class="d-flex flex-column gap-5 w-100">
          ...
          >
            <label class="w-100">Nature de l’évènement*</label>
            <el-select
              class="w-100"
              size="large"
              placeholder=""
          ...
          <el-form-item
            class=""
            prop="captionFr"
            v-if="offFieldType !== 'Congés' && offFieldType !== 'Voiture'"
          >
          ...
          <el-form-item class="" prop="description">
          ...
          <div
            ...
            v-if="
              (offFieldType === 'Congrès' && showButton) ||
              offFieldType === 'RP/STAFF'
            "
          >
    ...
    >
      ...
      >
        ...
        <span v-if="isEdit && !canUpdateActivity" @click="handleAdminContact">
...
const handleSubmitForm = async () => {
  if (
    (offFieldType.value !== "Congrès" || !showButton.value) &&
    offFieldType.value !== "RP/STAFF"
  ) {
    form.value.storyEvents.OffFieldActivityContact.storyEventInvolves = null;
  }
  const data = {
    ...form.value,
    storyEvents: Object.values(form.value.storyEvents).map((storyEvent) => {
      if (storyEvent.address && storyEvent.address.street) {
        storyEvent.address.street =
          storyEvent.address.street.charAt(0).toUpperCase() +
          storyEvent.address.street.slice(1);
      }
      return storyEvent;
    }),
  };
  if (props.isEdit) {
    await editStory({
      data: {
        ...data,
        storyEvents: data.storyEvents.map((e) => ({
          ...e,
          address: e.address ? { ...e.address, id: undefined } : undefined,
        })),
      },
      id: props.data.id,
    });
...
const offFieldType = computed(() => {
  if (!form.value.storyEvents?.OffFieldType?.listIdEventTypeValueObject?.id)
    return "";
  return lists.value.OffFieldTypes?.find(
    (t) =>
      t["id"] ===
      form.value.storyEvents.OffFieldType.listIdEventTypeValueObject.id
  ).captionFr;
});
...
onMounted(async () => {
  ...
  if (props.isEdit) {
    form.value = _.cloneDeep({
      ...props.data,
      storyEvents: {
        ...props.data.storyEvents,
        OffFieldActivityContact: {
          ...props.data.storyEvents?.OffFieldActivityContact,
          storyEventInvolves:
            props.data.storyEvents?.OffFieldActivityContact?.storyEventInvolves?.map(
              (item) => ({
                ...item,
